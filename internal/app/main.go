...
func printExitError(w io.Writer, globalOptions global.Options, code int, message string) {
	if globalOptions.JSON {
		...
		err := json.NewEncoder(w).Encode(jsonS)
		if err != nil {
			// ignore error as there's no good way to handle it
			_, _ = fmt.Fprintf(w, "JSON encode failed: %v\n", err)
...
// Main runs the restic command tree using the process's stdio and arguments,
// then terminates the process with the resulting exit code.
func Main() {
	Exit(Run(os.Args[1:], os.Stdin, os.Stdout, os.Stderr, nil))
}

// Run executes the restic command tree with the given arguments and IO streams.
// The optional preExec hook may adjust the root command or global options
// (e.g. change default flag values) right before execution. It returns the
// exit code instead of terminating the process, so embedders (such as the zap
// command) can reuse the full restic CLI in-process.
func Run(args []string, stdin io.ReadCloser, stdout, stderr io.Writer, preExec func(root *cobra.Command, gopts *global.Options)) int {
	tweakGoGC()
	...
	func() {
		term, cancel := termstatus.Setup(stdin, stdout, stderr, globalOptions.Quiet)
		defer cancel()
		globalOptions.Term = term
		ctx := createGlobalContext(stderr)
		root := newRootCommand(&globalOptions)
		if preExec != nil {
			preExec(root, &globalOptions)
		}
		root.SetArgs(args)
	...
	if exitCode != 0 {
		printExitError(stderr, globalOptions, exitCode, exitMessage)
	}
	return exitCode
}
